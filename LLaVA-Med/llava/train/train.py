...
from llava.train.train_utils import verify_mm_weights
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments, split: Tuple[float, float] = (0,1.0)):
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    if data_args.eval_from_train_set:
        ...
        train_dataset = LazySupervisedDataset(tokenizer=tokenizer,
                                    data_path=data_args.data_path,
                                    data_args=data_args)
        if data_args.eval_from_train_set is not None:
            eval_dataset = LazySupervisedDataset(tokenizer=tokenizer,
...
def train(attn_implementation=None):
    ...
    wandb.init(project="llava-med-mistral", name=training_args.output_dir.split("/")[-1]) 
    ...
    # NOTE: switched from LlavaMistralForCausalLM to LlavaMistralForCausalLM 
    print("Loading model...")
    # NOTE: given to delay_load=True the vision model does not load here. Projector is loaded (commented code checked for that).
    model = LlavaMistralForCausalLM.from_pretrained( # this checks if checkpoint has weights for vision tower and projector
    ...
    if model_args.vision_tower is not None:
        ...
        # Check weights are same as in checkpoint in HF
    ...
    #verify_mm_weights(model, "/vol/biomedic3/mv320/projects/VLMs/MEG_x_CL/LLaVA-Med/checkpoints/llava-med/model-00003-of-00004.safetensors")
