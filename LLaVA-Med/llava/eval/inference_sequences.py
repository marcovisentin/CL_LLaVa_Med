...
def main():
    ...
    parser.add_argument("--pipeline-base-output-dir", type=str, default="/vol/biomedic3/mv320/projects/VLMs/MEG_x_CL/LLaVA-Med/checkpoints/binary_sequential_finetuning_pipeline_",
                        help="Base directory where sequence folders (e.g., sequence_A) are stored. (PIPELINE_BASE_OUTPUT_DIR from training script)")
    parser.add_argument("--eval-script-path", type=str, default="/vol/biomedic3/mv320/projects/VLMs/MEG_x_CL/LLaVA-Med/llava/eval/inference_no_batch.py",
    ...
    parser.add_argument("--evaluation-output-dir", type=str, default="/vol/biomedic3/mv320/projects/VLMs/MEG_x_CL/LLaVA-Med/results",
    ...
    for item in os.listdir(args.pipeline_base_output_dir):
        ...
        if os.path.isdir(path) and (item.startswith("binary")):
